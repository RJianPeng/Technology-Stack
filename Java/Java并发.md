...
在Java内存模型中，线程需要将变量从主内存读取到工作内存中再进行操作，高并发的场景下，可能会出现线程读取脏数据到工作内存中的情况（即线程A读取了一个主内存中到变量x，线程B对变量x做了修改并赋值到主内存中，此时线程A的工作内存中仍然是x的脏数据）。volatile则会强制修改完变量后，立刻刷写到主内存并强制其他线程重新从主内存中读取变量，一定程度上避免了脏数据的问题，保证变量的可见性。但是volatile并不能保证对变量操作的原子性，所以不能保证线程安全。
