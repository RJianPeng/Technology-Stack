...
在Java内存模型中，线程需要将变量从主内存读取到工作内存中再进行操作，高并发的场景下，可能会出现线程读取脏数据到工作内存中的情况（即线程A读取了一个主内存中的变量x，线程B对变量x做了修改并赋值到主内存中，此时线程A的工作内存中仍然是x的脏数据）。volatile则会强制修改完变量后，立刻刷写到主内存并强制其他线程重新从主内存中读取变量，一定程度上避免了脏数据的问题，保证变量的可见性。但是volatile并不能保证对变量操作的原子性，所以不能保证线程安全。

### synchronized
（参考文档：https://blog.csdn.net/chachapaofan/article/details/105296257）
...
* 4，当线程B试图获得这个锁时，JVM发现同步锁处于偏向状态，但是Mark Word中的线程id记录的不是B，那么线程B会先用CAS操作试图获得锁，这里的获得锁操作是有可能成功的，因为线程A一般不会自动释放偏向锁,这个时候会去检查A线程是否活着，如果已经执行结束 ，则对象头会设置为无锁状态。如果抢锁成功，就把Mark Word里的线程id改为线程B的id，代表线程B获得了这个偏向锁，可以执行同步锁代码。如果抢锁失败，则继续执行步骤5。
...
#### synchronized一些锁优化
* 1.锁消除，即删除不必要的加锁操作，虚拟机在运行时，对于一些代码上要求同步，但是监测到不可能存在共享数据竞争的锁进行消除

* 2.锁粗化，如果一些列的连续操作都对同一个对象反复加锁和解锁，那即使没有出现线程竞争，虚拟机将会把加锁同步的番位扩展到整个操作序列的外部

* 3.自适应的自旋锁，即次数有限且不固定的自旋锁，每一次自旋的次数取决于上一次在该锁上的自旋时间和锁的状态，如果上一次自旋很少就成功，下一次自旋次数上限就会增加，反之就会减少，这样大大的节省了处理器资源。
...
* CachedThreadPool：一个任务创建一个线程；核心线程数为0，最大线程数是int的最大值。他的队列是synchronousQueue,没有缓冲区域的队列，最大线程数为int的最大值，容易出OOM，线程没有任务时会持续60秒
* FixedThreadPool：所有任务只能使用固定大小的线程；核心线程数和最大线程数相同；但是任务等待队列是int的最大值，容易出OOM
* SingleThreadExecutor：相当于大小为 1 的 FixedThreadPool;核心线程数和最大线程数都为1；但是任务等待队列是int的最大值，容易出OOM
