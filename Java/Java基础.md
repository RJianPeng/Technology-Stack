...
### synchronized
原理：每个对象头都有个监视器锁（monitor），当monitor被占用时会处于锁定状态，线程执行到synchronized的时候会尝试获取monitor的权限。线程进来的时候会先进行尝试性抢占然后再到等待队列中，所以是非公平的。获取的情况如下：
...
* 3.修饰变量时：表示这个变量为常量，不允许修改它的值。被修饰的变量只能在声明时或者构造器中赋值，且之后都无法修改它的值。
...
* 1.接口是用interface声明的
* 2.接口不能被实例化
* 3.接口中的方法都是抽象方法，Java8引入了使用default修饰的默认方法，这个引入主要是为了修改已经发布的接口代码并将对使用者的影响降到最低。
* 4.接口中不能包含静态方法，包括main方法
