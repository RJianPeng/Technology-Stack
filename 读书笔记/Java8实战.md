...
* [第十三章 函数式的思考](#第十三章函数式的思考)
* [第十四章 函数式编程的技巧](#第十四章函数式编程的技巧)
...
### 13.1.2 声明式编程
一般通过编程实现一个系统，有两种思考方式，一种是专注于如何实现，这种确定“如何做”的编程方式非常适合经典的面向对象编程，我们也称之为“命令式编程”。另一种方式则更关注要做什么，这种风格的编程通常称为声明式编程。用这种方式编写的代码更加接近问题陈述。

### 13.1.3 为什么要采用函数式编程
函数式编程具体实现了前面介绍的声明式编程和无副作用计算。这两个思想能帮助你更容易地构建和维护系统。



# 第十四章、函数式编程的技巧
### 14.2.2 科里化
科里化：可以将使用了6个参数的函数科里化为一个接受第2、4、6号参数，并返回一个接受5号参数的函数，这个函数又返回一个接受剩下的1、3号参数的函数。
例：
```
...
```
这种函数每次调用都需要传入三个参数，而且f，b可能会出现大量重复，这个时候可以定义工厂方法：
```
